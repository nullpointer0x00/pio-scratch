...
$CHAIN tx group vote 1 $ACCT1_KEY VOTE_OPTION_NO "no" $GAS_FLAGS --from $ACCT1_NAME -y -o json | jq
$CHAIN tx group vote 1 $ACCT2_KEY VOTE_OPTION_NO "no" $GAS_FLAGS --from $ACCT2_NAME -y -o json | jq